...
import { Validity } from "../globalFunctions";
...
const SignUp: React.FC<SignUpProps> = ({ navigation }) => {
	...
	const determineEye = (valid: boolean, show: boolean): React.JSX.Element => {
	...
	const validateEmail = (text: string) => {
		setEmail(text);
		setEmailValidity(Validity.validateEmail(email));
	};
	const validateUsername = (text: string) => {
		setUsername(text);
		setUsernameValidity(Validity.validateUsername(name));
	};
	const validatePassword = (text: string) => {
		setPassword(text.trim());
		setPasswordValidity(Validity.validatePassword(password));
	};
	const validatePhoneNumber = (text: string) => {
		setPhoneNumber(text.trim());
		setPhoneNumberValidity(Validity.validatePhoneNumber(phoneNumber));
	};
	const inputBoxes = [
		{
