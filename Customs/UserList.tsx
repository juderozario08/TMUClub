...
import { SetUsersByRole } from "../Globals/AppValues";
...
const UserList: React.FC<PropsWithChildren<UserListProps>> = ({
    ...
    const changeUser = async () => {
        await axios
            ...
            .then(() => {
                setUsers(
                    users.map((el) => (el._id === selectedUser._id ? selectedUser : el)),
                );
            })
            .catch((err) => {
    ...
    const deleteUser = async () => {
        await axios
            ...
            .then(() => {
                const people = users.filter((el) => el._id !== selectedUser._id);
                setUsers(people, role || ROLE.MEMBER);
